...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, NgxCsvParserModule, FileUploadComponent,  ConferenceImportComponent],
  template: `
  <form [formGroup]="conferenceForm" class="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md">
    <file-upload 
  ...
  <div class="mt-4">
    <app-conference-import [conferenceInput]="conferenceImportResponseSignal()"/>
  </div>
  `,
  styles: [`
    :host {
...
class ConferenceImportPageComponent {
  ...
  private filesControl = new FormControl<File[] | null>(null, FileUploadValidators.filesLimit(2));
  ...
  conferenceImportResponseSignal = toSignal(this.conferenceImport$.pipe(
