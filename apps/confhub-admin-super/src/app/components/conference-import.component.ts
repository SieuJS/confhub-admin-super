...
import { ConferenceResponse, ConferenceResponseItem } from '../models/response/conference.response';
...
export class ConferenceImportComponent {
  ...
  conference$ = combineLatest([this.debouncedFilterInput$,toObservable(this.perPage), toObservable(this.page), toObservable(this.sourceSelection), toObservable(this.conferenceInput)]) .pipe(
...
const filterBySearch =  (
  ...
  const filteredConferences = conferences.filter((conference) => {
    ...
    return (
      (acronym && acronym.toLowerCase().includes(search?.toLowerCase() || '')) ||
